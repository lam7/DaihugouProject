...
@IBDesignable class IDAProgressView: UIView{
    ///　メインバーの進歩率(0.0 - 1.0)
    ...
    @IBInspectable open var lineCap: String = convertFromCAShapeLayerLineCap(CAShapeLayerLineCap.butt) {
        didSet {
            progressLayer.lineCap = convertToCAShapeLayerLineCap(lineCap)
            auxiliaryLayer.lineCap = convertToCAShapeLayerLineCap(lineCap)
            guideLayer.lineCap = convertToCAShapeLayerLineCap(lineCap)
        }
    }
    ...
    @IBInspectable open var progressLayerColor: UIColor = .green
    
    @IBInspectable open var guideLayerColor: UIColor = .black
    ...
    open weak var delegate: IDAProgressViewDelegate?
    ...
    private(set) lazy var guideLayer: IDAShapeLayer = {
        ...
        guideLayer.strokeColor = guideLayerColor.cgColor
        return guideLayer
    }()
    ...
    public func set(progress: CGFloat, completion: (() -> ())? = nil){
        let m = min(progress, 1.0)
        let clipProgress = max(m, 0.0)
        let p = abs(clipProgress - self.progress)
        let duration = CFTimeInterval(p) * speed
        set(progress: progress, duration: duration, completion: completion)
    }
}

@objc protocol IDAProgressViewDelegate: class{
